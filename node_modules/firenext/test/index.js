...
var TEST_URL = 'https://firenext-test2.firebaseio.com',
...
describe('firebase-next specs', function() {
  ...
  describe('firebase', function() {
    ...
    it.only('should inc({ by: n, when: condition })', function() {
      return firenext.child('child4').inc({ by: 1, when: function() { return true; } })
      .then(function(v) {
        expect(v).to.eq(1);
        return firenext.child('child4').inc({ by: 1, when: function() { return false; } });
      })
      .then(function(v) {
        expect(v).to.eq(1);
        return firenext.child('child4').inc({ by: 1, when: function(n) { return n > 1; } });
      })
      .then(function(v) {
        expect(v).to.eq(1);
        return firenext.child('child4').inc({ by: -1, when: function(n) { return n === 1; } });
    ...
    it('should get lock / release lock', function() {
      ...
      return lockRef.getLock()
        .then(function() {
          ...
          // should happen after (once lock is released)
          return lockRef.getLock()
            .then(function() {
              return expect(foo.val()).to.eventually.eq(1);
            })
            .then(function() {
              return foo.inc(1);
            });
        ...
        .then(function() {
          return expect(foo.val()).to.eventually.eq(2);
        });
      });
  });
  ...
  describe('firebase query', function() {
    ...
    it('should exists query', function() {
      return expect(Promise.all([
        firenext.child('child1').exists(),
        firenext.child('childx').exists()
      ])).to.eventually.deep.eq([true, false]);
    });
  ...
  describe('firebase datasnapshot', function() {
    ...
    it('should map children keys', function() {
      return firenext.exec().then(function(children) {
        expect(children.keys()).to.deep.eq([ 'child1', 'child2' ]);
      });
    });

    it('should reduce children', function() {
      return firenext.exec().then(function(children) {
        var data = children.reduce(function(acc, child, i) {
          return  acc + i + '-' + child.key() + ';';
        }, '');

        expect(data).to.eq('0-child1;1-child2;');
      });
    });

    it('should get first child', function() {
      return firenext.exec().then(function(children) {
        var snap = children.firstChild();
        expect(snap.val()).to.deep.eq({ text: 'foo' });
      });
    });

    it('should filter children', function() {
      return firenext.exec().then(function(children) {
        var data = children.filter(function(snap) {
          return snap.child('text').val() === 'foo';
        }).map(function(snap) { return snap.val(); });
        expect(data).to.deep.eq([
          { text: 'foo' }
        ]);
      });
    ...
    it('should compare two snapshots', function() {
      Promise.all([
        firenext.child('alanisawesome').set('alanisawesome'),
        firenext.child('gracehop').set('gracehop'),
        firenext.child('physicsmarie').set('physicsmarie', 1),
        firenext.child('dennisplusplus').set('dennisplusplus', 2),
        firenext.child('doublehelix').setWithPriority('doublehelix', 'dna'),
      ]).then(function(arr) {
